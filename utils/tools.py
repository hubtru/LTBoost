...
def visual(true, preds=None, name='./pic/test.pdf', seq_len=0, title=""):
    """
    Results visualization
    """
    plt.figure()
    plt.plot(true, label='GroundTruth', linewidth=1.5)
    if preds is not None:
        plt.plot(list(range(seq_len,len(preds))), preds[seq_len:], label='Prediction', color='orange', linewidth=1)
    
    if seq_len:
        plt.axvline(x=seq_len, color="k")
